...
import weka.attributeSelection.ConsistencySubsetEval.hashKey;
...
public class PreProcessing {
	...
	public static Instances process(LinkedList<EncounterIdConceptDrug> data) {
		...
		// hash tables for concepts and drugs with key as the concept_id and drug_id
		// concept hashtable contains the value as the column index of concept attribute in instances
		...
		// nominal vector that contains all drugIds for the drug attribute in Instance
		FastVector drugs = new FastVector();
		
		// nominal vector of attributes of the instances
		FastVector attrib = new FastVector();
		
		// nominal vector of values for concepts which is "true" if it exist or value is missing 
		...
		//loop where we find distinct concept_id's, drug_id's and total number of distinct encounter_id
		while (itr.hasNext()) {
			temp = itr.next();
			...
			if (!drugTable.containsKey(temp.getDrug())) {
				...
				// add new value to nominal vector of drugs as you find different drugs
				drugs.addElement(Integer.toString(temp.getDrug()));
			}
			
			// increase the counter for number of encounter_id's as you get a id different from previous one
	...
	/*
	 * Method that returns the instances after filtering the concepts that are very frequent
	 */
